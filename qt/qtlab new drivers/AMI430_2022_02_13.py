...
class AMI430_2022_02_13(Instrument):
    ...
    COILCONSTANT = 0.1078       #T/A              #0.0510 for Bx            #0.09279 for the center fridge
    ...
    CURRENTRATING = 83.49      #A            # 78 for Bx
    ...
    CURRENTRAMPLIMIT = 0.0665     #A/s    0.0025 for Bx
    ...
    def __init__(self, name, address, port=7180):
    ...
    def do_get_ID(self):
        id = self._ask('*IDN?\n').strip().replace('AMERICAN MAGNETICS INC.,MODEL 430', 'AMI 430')
        name = self._ask('IPNAME?\n').strip()
        return ('%s (%s)'%(id,name))
