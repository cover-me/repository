...
class Keithley2450_20230221(Instrument):
    ...
    def __init__(self, name, address):
        ...
        self.add_parameter('source_i_level', type=types.FloatType, flags=Instrument.FLAG_GETSET, 
                            maxstep=3e-9, stepdelay=100, units='A')
        self.add_parameter('source_v_level', type=types.FloatType, flags=Instrument.FLAG_GETSET, 
                            maxstep=3e-3, stepdelay=100, units='V')
        self.add_parameter('compliance', type=types.FloatType, 
                            flags=Instrument.FLAG_GETSET|Instrument.FLAG_GET_AFTER_SET)
        self.add_parameter('nplc', type=types.FloatType, flags=Instrument.FLAG_GETSET)
    ...
    def do_get_compliance(self):# compliance
        stype = self.get_source_type()
        if self.is_2450:
            d = {'VOLT':'SOUR:VOLT:ILIM?','CURR':'SOUR:CURR:VLIM?'}
        else:
            d = {'VOLT':':SENS:CURR:PROT?','CURR':':SENS:VOLT:PROT?'}
        if stype in d:
            return float(self._query(d[stype]))
        else:
            print 'Unknown source type!'
    ...
    def do_set_compliance(self, x):
        stype = self.get_source_type()
        if self.is_2450:
            d = {'VOLT':'SOUR:VOLT:ILIM %s','CURR':'SOUR:CURR:VLIM %s'}
        else:
            d = {'VOLT':':SENS:CURR:PROT %s','CURR':':SENS:VOLT:PROT %s'}
        if stype in d:
            self._execute(d[stype]%x)
        else:
            print 'Unknown source type!'            

    ## sense    
    def do_get_nplc(self):# nplc
        sen_type = self.get_sense_type()
        sen_type = sen_type.strip('"').split(':')[0]
        if sen_type in ['VOLT', 'CURR']:
            return float(self._query(':SENS:%s:NPLC?'%sen_type))
        else:
            print 'Sense type not supported!' 
    
    def do_set_nplc(self,n):
        sen_type = self.get_sense_type()
        sen_type = sen_type.strip('"').split(':')[0]
        if sen_type in ['VOLT', 'CURR']:
            return self._execute(':SENS:%s:NPLC %s'%(sense_type,n))
        else:
            print 'Sense type not supported!' 
