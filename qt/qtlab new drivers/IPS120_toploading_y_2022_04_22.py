...
class IPS120_toploading_y_2022_04_22(Instrument):
    '''
...
#TODO: get doesn't always update the wrapper! (e.g. when input is an int and output is a string)
    ...
    def __init__(self, name, address):
        ...
        #self._number = number number is not used in GPIB mode
        self._visainstrument = visa.instrument(self._address, term_chars = '\r')
        self._visainstrument.clear()
        self._execute('C3')
        self._values = {}
        #self._visainstrument.stop_bits = 2 not used in GPIB mode
        ...
        self.add_parameter('field_setpoint', type=types.FloatType,
            flags=Instrument.FLAG_GETSET | Instrument.FLAG_GET_AFTER_SET,
            minval=-self.MAX_FIELD, maxval=self.MAX_FIELD)
        self.add_parameter('field', type=types.FloatType,
            flags=Instrument.FLAG_GETSET | Instrument.FLAG_GET_AFTER_SET,
            minval=-self.MAX_FIELD, maxval=self.MAX_FIELD)
        self.add_parameter('rampRate', type=types.FloatType,
            flags=Instrument.FLAG_GETSET | Instrument.FLAG_GET_AFTER_SET,
            minval=0, maxval=self.MAX_FIELD_RATE)
        self.add_parameter('current_setpoint', type=types.FloatType,
            flags=Instrument.FLAG_GETSET | Instrument.FLAG_GET_AFTER_SET,
            minval=-self.AMP_PER_TESLA * self.MAX_FIELD, maxval=self.AMP_PER_TESLA * self.MAX_FIELD)

        self.add_parameter('sweeprate_current', type=types.FloatType,
            flags=Instrument.FLAG_GETSET | Instrument.FLAG_GET_AFTER_SET,
            minval=-self.AMP_PER_TESLA * self.MAX_FIELD_RATE, maxval=self.AMP_PER_TESLA * self.MAX_FIELD_RATE)
    ...
    def do_set_sweeprate_current(self, sweeprate):
        ...
        self.get_rampRate() # Update rampRate
    ...
    def do_set_field(self, field):
        ...
        if self.get_switch_heater()==1:
            logging.info(__name__ + ' : Setting field to %s' % field)
            self._execute('J%s' % field)
            self.get_current_setpoint() #Update current setpoint
            self.get_field_setpoint()
            while math.fabs(field - self.get_field()) > self.MARGIN:
                sleep(0.050)
            return True
        else:
            print 'Can not set field while heat is OFF!'
    ...
    def do_set_rampRate(self, sweeprate):
        '''
